...
namespace ToMau
{
    class QueueLinearFloodFill: IStrategy
    {
        ...
        void Process(int _x, int _y)
        {
            ...
            while (queue.Count > 0)
            {
                    ...
                    && CheckPixel(ref idx))
                {
                    ...
                    if (!havePointUpQueue)
                        if (checkSeed(new Point(P.X + increaseX, P.Y + 1), queue)) havePointUpQueue = true;
                    if (!havePointDownQueue)
                        if (checkSeed(new Point(P.X + increaseX, P.Y - 1), queue)) havePointDownQueue = true;
        ...
        bool checkSeed(Point _p, Queue<Point> _queue)
        {
            int x, y;
            x = _p.X; y = _p.Y;
            int idx = CoordsToByteIndex(ref x, ref y);
